...
class simple_skip_connection(nn.Module):
    def __init__(self, args):
        super(simple_skip_connection, self).__init__()
        self.classification = args.classification
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(True),
        ...
        self.layer6 = torch.nn.Sequential(
            torch.nn.Conv2d(640, 256, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(True),
            torch.nn.Upsample(scale_factor=2),
            torch.nn.BatchNorm2d(256)
        )
        self.layer7 = torch.nn.Sequential(
            torch.nn.Conv2d(320, 256, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(True),
            torch.nn.Upsample(scale_factor=2),
            torch.nn.BatchNorm2d(256)
        )
        ...
        self.layer8 = torch.nn.Sequential(
            torch.nn.Conv2d(257, 128, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(negative_slope=0.2)
        )
        self.classification_out = torch.nn.Conv2d(257, 313, kernel_size=1, padding=0, dilation=1, stride=1)
        self.regression_out = torch.nn.Conv2d(128, 2, kernel_size=1, padding=0, dilation=1, stride=1)
    ...
    def forward(self, x):
        ...
        layer_5_output = self.layer5(layer_4_output)
        layer_6_output = self.layer6(torch.cat((layer_2_output, layer_5_output), dim=1))
        layer_7_output = self.layer7(torch.cat((layer_1_output, layer_6_output), dim=1))
        if self.classification:
            output = self.classification_out(torch.cat((x, layer_7_output), dim=1))
        else:
            output = self.layer8(torch.cat((x, layer_7_output), dim=1))
            output = self.regression_out(output)
        return output
