...
def main():
    ...
    if not args.from_npy:
        ...
        train_L, train_ab = cvt2lab_new(x_train)
        np.save('./data/x_train.npy', train_L)
        np.save('./data/y_train.npy', train_ab)
        test_L, test_ab = cvt2lab_new(x_test)
        np.save('./data/x_valid.npy', test_L)
        test_rgb = np.rollaxis(x_test, 1, 4)
        test_rgb = test_rgb / 255
        np.save('./data/rgb_valid.npy', test_rgb)
    ...
    test_RGB_gt = np.load('./data/rgb_valid.npy')
    ...
    if args.classification:
        ...
        if args.skip_connection:
            model_path = './checkpoints/classification_with_sc.pth'
        else:
            model_path = './checkpoints/classification.pth'
        cp_path = './checkpoints/classification_best_model_'
    else:
        criterion = nn.MSELoss()
        if args.skip_connection:
            model_path = './checkpoints/regression_with_sc.pth'
        else:
            model_path = './checkpoints/regression.pth'
        cp_path = './checkpoints/regression_best_model_'
